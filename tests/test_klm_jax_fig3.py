...
def test_run_fig3_experiment_returns_smoke_rows():
    rows = run_fig3_experiment(quick_config())
    ...
    assert first["reference_power"] == 12
    assert first["reference_step"] == 2.0 ** (-12)
    assert first["number_of_fine_steps"] == 2**12